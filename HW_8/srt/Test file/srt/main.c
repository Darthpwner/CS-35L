...
int nthreads;
scene_t scene;
float image[width][height][3]; //3-D array
struct t_data {
  int id;
};
//////////////////////////
...
{
  nthreads = argc == 2 ? atoi( argv[1] ) : 0; //Converts from string to int
    ...
    /////////////
    //Allocate additional memory using malloc to hold pthreads
    pthread_t *tids = (pthread_t*) malloc(sizeof(pthread_t) * nthreads); 
    long i; //Used to iterate through the "tids" array
    //Array to hold all the data for the thread id's
    struct t_data* data_arr = malloc(sizeof(struct t_data) * nthreads);

     for (i = 0; i < nthreads; i++)
      {
	data_arr[i].id = i;
	int ret = pthread_create(&tids[i], 0, ray_tracing, &data_arr[i]);
	if(ret)
	  {
...
void *ray_tracing(void *arg)
{
 Vec3 camera_pos;
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
    ...
    ////////////////////
    for( int px=data->id; px<width; px += nthreads )
      {
