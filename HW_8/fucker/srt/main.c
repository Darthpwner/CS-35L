...
scene_t scene; //threadFunc can access scene now

float pixelColor[width][height][3]; //Global Variable

void* ThreadFunction(void *arg)
{
  info* threadInfo = (info *) arg;
  int width_begin = threadInfo->widthBegin;
  int width_size = threadInfo->widthSize;
  //printf("%d, %d",width_begin,width_size);
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
	...
	: pixel_dx;
	  ...
	  /* for every pixel */
	  for( int px=width_begin; px<(width_begin+width_size); ++px )
	    {
	      ...
	      for( int py=0; py<height; ++py )
		{
		  ...
		  pixelColor[px][py][0] = scaled_color[0];
		  pixelColor[px][py][1] = scaled_color[1];
		  pixelColor[px][py][2] = scaled_color[2];
		}
	    }
	  pthread_exit(0);
}
...
main( int argc, char **argv )
{
  ...
  //Dividing work to each thread now

  int widthSize = width/nthreads;
  int widthRemains = width % nthreads;  //leftovers can occur
  int ifRemains = 0;

  if (widthRemains > 0)    //If there are any leftovers......
    ifRemains = 1;

  pthread_t* threadID = (pthread_t*) malloc(sizeof(pthread_t) *(nthreads+ifRemains));
  //pthread_t threadID[nthreads+ifRemains];

  info* threadSample = (info*)malloc(sizeof(info)*(nthreads+ifRemains));
  //struct info* threadSample = malloc(sizeof(struct info*)*(nthreads+ifRemains));


  //threadSamplewidth_size = widthSize;
  int t;
  int rs;

  for(t = 0; t < nthreads; t++) {
    threadSample[t].widthBegin = t*widthSize;
    threadSample[t].widthSize = widthSize;
    //printf("%d, %d",threadSample[t].widthBegin,threadSample[t].widthSize);
    rs = pthread_create(&(threadID[t]), 0, ThreadFunction, (void*)&threadSample[t]);
    if(rs) {
  ...
  if (ifRemains){//for the remainder
    threadSample[nthreads].widthSize = widthRemains;
    threadSample[nthreads].widthBegin = nthreads*widthSize;
    //printf("%d, %d",threadSample[nthreads].widthBegin,threadSample[nthreads].widthSize);
    rs = pthread_create(&(threadID[nthreads]), 0, ThreadFunction, (void*)&threadSample[nthreads]);
  }
  ...
  for(t = 0; t < (nthreads+ifRemains); t++) {
    //void *retVal;
    rs = pthread_join(threadID[t], NULL);
    if(rs) {
  ...
  for (int i = 0; i < width ; i++){    //Print out pixel colors
    for (int j = 0; j < height; j++){
      printf( "%.0f %.0f %.0f\n", pixelColor[i][j][0], pixelColor[i][j][1], pixelColor[i][j][2] );
    }
    printf("\n");
  }
